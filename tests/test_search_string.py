...
from src.serch_string import count_operations_by_category, filter_by_description
...
@pytest.mark.parametrize(
    "data_category, output_data, category",
    [
        (
            [
                {"id": 1, "description": "Перевод на счет", "amount": 100},
            ...
            {"Перевод": 2, "Покупка": 3, "Оплата": 1},
            ["Перевод", "Оплата", "Покупка"],
        )
    ],
)
def test_count_operations_by_category(data_category, output_data, category):
    """Функция для тестирования возврата словаря, в котором ключи — это названия категорий,
